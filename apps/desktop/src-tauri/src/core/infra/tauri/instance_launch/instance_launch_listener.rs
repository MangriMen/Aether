...
use crate::core::WindowManagerState;
...
pub fn instance_launch_listener(
    ...
    tauri::async_runtime::spawn(async move {
        match event.event {
            ProcessEventType::Launched => handle_instance_launch(app_handle, window_manager).await,
            ProcessEventType::Finished => handle_instance_finish(app_handle, window_manager).await,
        }
    });
}
